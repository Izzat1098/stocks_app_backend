...
from .util import (
    ...
    validate_sector_optional,
)


class ExchangeCreate(BaseModel):
    abbreviation: str = Field(
        ..., max_length=10, description="Exchange abbreviation (e.g., NYSE)"
    )
    name: str = Field(..., max_length=100, description="Full exchange name")
    country: str = Field(
        ..., max_length=50, description="Country name (e.g., United States, Malaysia)"
    )
...
class StockCreate(BaseModel):
    ticker: str = Field(
        ..., max_length=10, description="Stock ticker symbol (e.g., AAPL)"
    )
    company_name: str = Field(..., max_length=50, description="Full company name")
    abbreviation: Optional[str] = Field(
        None, max_length=10, description="Company abbreviation"
    )
    exchange_id: Optional[int] = Field(
        None, gt=0, description="ID of the exchange where stock is listed"
    )
    sector: Optional[str] = Field(
        None, max_length=50, description="Business sector code"
    )
    country: Optional[str] = Field(
        None, max_length=50, description="Country name (e.g., United States, Malaysia)"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Detailed company description"
    )
    ai_description: Optional[str] = Field(
        None, max_length=500, description="AI-generated description"
    )
